...
_FunParseSection(
	...
	*block = MAX( 1, strtol( s3, &s, 0 ) );
    ...
    else if ( sscanf( f, "%[-0-9.*] : %[-0-9.*]%n", s1, s2, &n ) == 2 ) {
	tx0 = ( *s1 == '*' ) ? nan : atof( s1 );
	tx1 = ( *s2 == '*' ) ? nan : atof( s2 );
	ty0 = tx0;
	ty1 = tx1;
    ...
    else if ( sscanf( f, "%[*] , %[*] , %[0-9as]%n", s1, s2, s3, &n ) == 3 ) {
	/* sanity check */
	if ( fun->dims == 1 ) {
	    gerror( stderr, "can't specify a 2D section with a 1D image\n" );
	}
	*block = MAX( 1, strtol( s3, &s, 0 ) );
    ...
    else if ( sscanf( f, "%[*] , %[0-9as]%n", s1, s2, &n ) == 2 ) {
	*block = MAX( 1, strtol( s2, &s, 0 ) );
	if ( s && *s && ( tolower( *s ) == 'a' ) )
	    *btype = FUN_AVG;
	f += n;
    }
    else if ( sscanf( f, "%[*] , %[*]%n", s1, s2, &n ) == 2 ) {
	/* sanity check */
	if ( fun->dims == 1 ) {
	    gerror( stderr, "can't specify a 2D section with a 1D image\n" );
	}
