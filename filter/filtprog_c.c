...
_FilterInitString(
	...
	if ( strncmp( t, "annulus(", 8 ) &&
	     ...
	     strncmp( t, "vpoint(", 7 ) &&
	     strncmp( t, "polygon(", 8 ) &&
	     strncmp( t, "field(", 6 ) && strncmp( t, "imagemask(", 10 ) ) {
	    fptr = t;
	    continue;
	}
...
FilterProgOpen_C(
    ...
    if ( !( ccstr = getenv( "FILTER_CC" ) ) &&
         !( ccstr = getenv( "CC" ) ) && !( ccstr = FILTER_CC ) ) {
	ccstr = "gcc";
    }
    ...
    snprintf( tbuf, SZ_LINE,
              "$FILTER_OBJDIR:$FILTER_LIBDIR:%s:$HOME/.funtools:$SAORD_ROOT/lib:/usr/lib:/usr/local/lib:/opt/local/lib",
              OBJPATH );
    switch ( filter->type ) {
	case TYPE_EVENTS:
		...
		switch ( sp->type ) {
			...
			if ( fhd->table->col[sp->idx].n == 1 ) {
				...
				switch ( fhd->table->col[sp->idx].n ) {
				    ...
				    case 32:
					fprintf( fd,
					         "#define %s %s*((%s *)(SW4(eptr+%d,%d,_swf,%d)))%s\n",
					         sp->name, s, "unsigned int",
					         offset, 4, offset, t );
					break;
				    default:
					fprintf( fd,
