     ...
     char *argv[];
{
	...
	if ( argv[i][0] == '+' ) {
		...
		while ( *s ) {
			...
			case 'U':
			    unique = 1;
			    differ = 1;
			    break;      /* Unique lines only    */
			case 'D':
			    unique = 1;
			    differ = 2;
			    break;      /* Duplic lines only    */
			case 'A':
			    unique = 1;
			    differ = 4;
			    break;      /* All Duplic lines     */
			case 'S':
			    unique = 1;
			    differ = 3;
			    break;      /* Set counter          */
	...
	for ( key = keyhead.next; key; key = key->next )
	    if ( !key->ignore && !key->translate && !key->skipsblanks
	         && !key->reverse && !key->skipeblanks && !key->month
	         && !key->numeric && !key->nzone ) {
	...
	if ( !keyhead.next
	     && ( gkey.ignore || gkey.translate || gkey.skipsblanks
