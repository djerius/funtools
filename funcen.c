...
#endif
{
    ...
    *cregionfile = '\0';
	...
	switch ( ++wcnt ) {
		...
		if ( strstr
		     ( "j2000|fk5|icrs|b1950|fk4|galactic|ecliptic|linear|image|physical|amplifier|detector",
		       tbuf ) ) {
	...
	if ( domem || doimfilt ) {
		...
		if ( ( x1 - x0 <= 0 ) || ( y1 - y0 <= 0 ) )
		    gerror( stderr,
		            "invalid or zero image dimensions(s) for table\n",
