...
#endif
{
	...
	case FUN_EVENTS:
	    ...
	    /* get tlmin/tlmax, etc. */
	    GetArgs( argc, argv, optind + 2, &tlmin, &tlmax, &dval, &got1,
	             &got2 );
	    if ( dowidth && ( dval <= 0 ) )
	    ...
	    FunColumnSelect( fun, sizeof( double ), NULL, col, "D", "r", 0,
	                     NULL );
	    /* for fp data, we cannot determine obins, so choose an arbitrary value */
	    if ( !dowidth && ( ( dtype == 'D' ) || ( dtype == 'E' ) )
	         && ( obins <= 0 ) )
		obins = 128;
		...
		if ( obins <= 0 )
		    gerror( stderr,
		            "calculated number of bins (%d) must be positive\n",
		            obins );
		tlwidth = ( double ) obins *tlinc;
		if ( tlwidth <= 0 )
		    gerror( stderr,
		            "calculated bin width (%.8f) must be positive\n",
		            tlwidth );
		tlmax = tlmin + tlwidth;
	    }
	    else {
		...
		fprintf( stdout,
		         ...
		         "bin", cold, "value", cold, "lo_edge", cold,
		         "hi_edge" );
		fprintf( stdout, "%6.6s%c%21.21s%c%21.21s%c%21.21s\n", dash,
		         cold, dash, cold, dash, cold, dash );
	    }
	    else {
		fprintf( stdout,
		         "%6s%c%9s%c%21s%c%21s\n",
		         "bin", cold, "value", cold, "lo_edge", cold,
		         "hi_edge" );
		fprintf( stdout, "%6.6s%c%9.9s%c%21.21s%c%21.21s\n", dash,
		         cold, dash, cold, dash, cold, dash );
	    }
	    for ( i = 0; i < obins; i++ ) {
		lo = tlmin + ( i * tlinc );
		...
		if ( donorm ) {
		    fprintf( stdout, "%6d%c%21.8f%c%21.8f%c%21.8f\n",
		             i + 1, cold, buf[i] / tlinc, cold, lo, cold,
		             hi );
		}
		else {
		    fprintf( stdout, "%6d%c%9d%c%21.8f%c%21.8f\n",
		             i + 1, cold, ( int ) buf[i], cold, lo, cold,
		             hi );
		}
	    }
	    /* free up buffers */
	...
	case FUN_ARRAY:
	    ...
	    if ( dowidth )
		gerror( stderr,
	                "specifying bin width (-w) not supported with images\n" );
	    ...
	    if ( doproj ) {
		/* get tlmin/tlmax, etc. */
		GetArgs( argc, argv, arg, &tlmin, &tlmax, &dval, &got1,
		         &got2 );
		obins = ( int ) dval;
		FunInfoGet( fun,
		...
		fprintf( stdout,
		         "%6s%c%21s%c%21s%c%21s\n",
		         "bin", cold, "value", cold, "lo_bin", cold,
		         "hi_bin" );
		fprintf( stdout, "%6.6s%c%21.21s%c%21.21s%c%21.21s\n", dash,
		         cold, dash, cold, dash, cold, dash );
		for ( i = 0; i < obins; i++ ) {
		    fprintf( stdout, "%6d%c%21.8f%c%21.8f%c%21.8f\n",
		             i + 1, cold, buf[i], cold,
		             ceil( tlmin + ( i * tlinc ) ), cold,
		             ceil( tlmin + ( ( i + 1 ) * tlinc ) ) - 1 );
		}
		/* free up buffers */
		if ( buf ) xfree( buf );
		if ( rbuf ) xfree( rbuf );
		...
		if ( !got1 || !got2 ) {
			...
			if ( nmask ) {
				...
				while ( 1 ) {
				    ...
				    for ( ix = masks[in].xstart;
				          ix <= masks[in].xstop; ix++ ) {
					i = ( ( iy - 1 ) * dim1 ) + ( ix -
					                              1 );
					if ( rbuf[i] < tlmin )
					    tlmin = rbuf[i];
		...
		if ( gotimage ) {
			...
			for ( i = 0; i < dim2; i++ ) {
			    if ( !
			         ( rbuf =
			           FunImageRowGet( fun, rbuf, i + 1, i + 1,
			                           "bitpix=-64" ) ) )
				gerror( stderr,
			                "can't FunImageRowGet: %d %s\n",
			                i + 1, iname );
		...
		fprintf( stdout,
		         "%6s%c%21s%c%21s%c%21s\n",
		         "bin", cold, "value", cold, "lo_val", cold,
		         "hi_val" );
		fprintf( stdout, "%6.6s%c%21.21s%c%21.21s%c%21.21s\n", dash,
		         cold, dash, cold, dash, cold, dash );
		for ( i = 0; i < obins; i++ ) {
