...
main(
	...
	case FUN_EVENTS:
	    ...
	    if ( !( idx = FunColumnLookup( fun, col, 0, NULL, &dtype, NULL, NULL, NULL, NULL ) ) )
	...
	case FUN_ARRAY:
		...
		FunInfoGet( fun,
		            ...
		            FUN_BINCOLS, &bincols, FUN_SECT_DIM1, &dim1, FUN_SECT_DIM2, &dim2, 0 );
		/* set dim2 to 1 for a 1D image */
		if ( dim2 == 0 ) dim2 = 1;
		...
		if ( gotimage ) {
			...
			for ( i = 0; i < dim2; i++ ) {
			    if ( !
			         ( rbuf =
			           FunImageRowGet( fun, rbuf, i + 1, i + 1, "bitpix=-64" ) ) )
				gerror( stderr, "can't FunImageRowGet: %d %s\n", i + 1, iname );
			    /* bin up all rows */
			    for ( j = 0; j < dim1; j++ ) {
			        if ( ( rbuf[j] < tlmin )
