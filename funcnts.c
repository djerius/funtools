...
main(
	...
	*tbuf2 = '\0';
	...
	if ( name[BKG] ) {
	    ...
	    FunInfoGet( fun[BKG], FUN_ENDIAN, &endian[BKG],
	                ...
	                FUN_RAWSIZE, &rawsize[BKG], FUN_BINCOLS,
	                &bincols[BKG], FUN_ROWSIZE, &rowsize[BKG], 0 );
	    /* set dim2 to 1 for a 1D image */
	...
	switch ( type[SRC] ) {
		...
		if ( evimage ) {
		    ...
		    snprintf( mode, SZ_LINE,
		              "type=events,convert=%s,evsect=\"%d %d %d %d %d\"",
		              ( endian[SRC] ==
		                is_bigendian(  ) )? "false" : "true", x0[SRC],
		              x1[SRC], y0[SRC], y1[SRC], block[SRC] );
		    /* add columns */
		    if ( bincols[SRC] ) {
			strcat( mode, "," );
			strcat( mode, bincols[SRC] );
  ...
  intvagain:
	...
	fprintf( stdout, "#   data_file:\t\t%s\n", name[SRC] );
	if ( valname[SRC] && valname[BKG]
	     && strcmp( valname[SRC], valname[BKG] )
	     && ( *valname[BKG] != '-' ) )
  ...
  sumagain:
	...
	if ( area[SRC][i] ) {
		...
		for ( j = 0; j < 4; j++ ) {
			...
			if ( ( j < 2 ) && !dopixels && ( dpp[SRC] > 0.0 ) )
			    fprintf( stdout, "%c%9.3f", cold,
			             ( dval * dpp[SRC] * ARCSEC_PER_DEG ) );
			else
	...
	/* might have to display zero area pixels */
	    ...
	    fprintf( stdout, fmt, i + 1, cold, 0.0, cold, 0.0, cold, 0.0,
	             cold, 0.0, cold, 0.0, cold, 0.0, cold, 0.0 );
