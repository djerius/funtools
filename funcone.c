...
NextList(
    ...
    if ( !( list->row = FunTableRowGet( list->fun, NULL, 1, NULL, &got ) )
         || !got )
	return 0;
	...
	snprintf( rastr, SZ_LINE - 1, "%15.10f%c",
                  DConvert( list->row,
                            ...
                            list->csys->value[0].n ), list->csys->type[0] );
    /* dec */
    if ( list->lits[1] )
	...
	snprintf( decstr, SZ_LINE - 1, "%15.10f%c",
                  DConvert( list->row,
                            ...
                            list->csys->value[1].n ), list->csys->type[1] );
    /* radius */
    if ( list->lits[2] )
	...
	snprintf( radstr, SZ_LINE - 1, "%15.10f%c",
                  DConvert( list->row,
                            ...
                            list->csys->value[2].n ), list->csys->type[2] );
