...
hgetdate(
	...
	/* New FITS date format: yyyy-mm-ddThh:mm:ss[.sss] */
	    ...
	    *dval = ( double ) year + ( ( double ) yday / yeardays );
		...
		fday =
		    ( ( 3.6e3 * ( double ) hours ) +
		      ( 6.e1 * ( double ) minutes ) + seconds ) / 8.64e4;
...
/* Return pointer to calling program */
	...
	if ( get_fits_head_str( keyword, iel, ip, &nel, &np, &ier, hstring )
	     != NULL )
	    return ( hstring );
	else
	    return ( NULL );
...
isnum(
	...
	if ( ( cstr < 48 || cstr > 57 ) &&
		...
		cstr1 = string[i - 1];
		cstr2 = string[i + 1];
		if ( cstr == '-' && cstr1 > 47 && cstr1 < 58 && cstr2 > 47 && cstr2 < 58 )
		    return ( 4 );
		else if ( cstr1 != 'D' && cstr1 != 'd' &&
		          cstr1 != 'E' && cstr1 != 'e' && cstr1 != ':' && cstr1 != ' ' )
		    return ( 0 );
	    }
	}
...
strfix(
	...
	if ( isnum( string ) > 1 && strchr( string, '.' ) != NULL &&
	     ...
	     strsrch( string, "E+" ) == NULL &&
	     strsrch( string, "e-" ) == NULL && strsrch( string, "e+" ) == NULL ) {
	    lstr = strlen( string );
	    s = string + lstr - 1;
	    while ( *s == '0' && lstr > 1 ) {
		if ( *( s - 1 ) != '.' ) {
