     ...
     char *option;
{
	...
	switch ( csys->data[i] ) {
		...
		break;
	    case 'd':
		csys->axis[i].wp = 360.0;
		break;
	    case 'r':
		csys->axis[i].wp = X_2PI;
		break;
	    default:
     ...
     TableRow row;
{
    ...
    switch ( csys->data[0] ) {
	case 'h':
	    ra = table_colvald( row, csys->axis[0].c1 );
	    break;
	case 'd':
	    ra = d2h( table_colvald( row, csys->axis[0].c1 ) );
	    break;
	case 'r':
	    ra = r2h( table_colvald( row, csys->axis[0].c1 ) );
	    break;
    }
    switch ( csys->data[1] ) {
	case 'h':
	    dec = h2d( table_colvald( row, csys->axis[1].c1 ) );
	    break;
	case 'd':
	    dec = table_colvald( row, csys->axis[1].c1 );
	    break;
	case 'r':
	    dec = r2d( table_colvald( row, csys->axis[1].c1 ) );
	    break;
    }
    switch ( csys->data[2] ) {
	case 'h':
	    width = h2d( csys->range[0] );
	    break;
	case 'd':
	    width = csys->range[0];
	    break;
	case 'r':
	    width = r2d( csys->range[0] );
	    break;
    }
     ...
     TableRow row2;
{
    ...
    switch ( csys->data[0] ) {
	case 'h':
	    r1 = h2r( table_colvald( row1, csys->axis[0].c1 ) );
	    r2 = h2r( table_colvald( row2, csys->axis[0].c2 ) );
	    break;
	case 'd':
	    r1 = d2r( table_colvald( row1, csys->axis[0].c1 ) );
	    r2 = d2r( table_colvald( row2, csys->axis[0].c2 ) );
	    break;
	case 'r':
	    r1 = table_colvald( row1, csys->axis[0].c1 );
	    r2 = table_colvald( row2, csys->axis[0].c2 );
	    break;
    }
    switch ( csys->data[1] ) {
	case 'h':
	    d1 = h2r( table_colvald( row1, csys->axis[1].c1 ) );
	    d2 = h2r( table_colvald( row2, csys->axis[1].c2 ) );
	    break;
	case 'd':
	    d1 = d2r( table_colvald( row1, csys->axis[1].c1 ) );
	    d2 = d2r( table_colvald( row2, csys->axis[1].c2 ) );
	    break;
	case 'r':
	    d1 = table_colvald( row1, csys->axis[1].c1 );
	    d2 = table_colvald( row2, csys->axis[1].c2 );
	    break;
    }
    switch ( csys->data[2] ) {
	case 'h':
	    radius = h2r( csys->range[0] );
	    break;
	case 'd':
	    radius = d2r( csys->range[0] );
	    break;
	case 'r':
	    radius = csys->range[0];
	    break;
     ...
     TableRow row;
{
    ...
    csys->axis[0].R =
        range_union( table_svalue
                     ( csys->axis[0].search, csys->axis[0].value,
     ...
     char *argv[];
{
	...
	if ( argv[i][0] == '-' && argv[i][1] == 'i' ) {
		...
		except( ex_search, "search: can't open input file: %s : %s",
	...
	if ( argv[n][0] == '-' ) {
		...
		case 'o':
			...
			except( ex_search, "can't open output file: %s : %s",
	...
	while ( T1 == NULL
		...
		switch ( axes[i]->type ) {
		    ...
		    case '2':{
			    double hi = axes[i]->hi;
			    double lo = axes[i]->lo;
			    double wp = axes[i]->wp ? axes[i]->wp : 0.0;
