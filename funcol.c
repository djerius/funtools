...
_FunColumnSelect(
    ...
    if ( _FunKeyword( mbuf, "org", NULL, tbuf, SZ_LINE ) ) {
	    ...
	    fun->org = FUN_ORG_SOA;
	          ...
	          || !strcasecmp( tbuf, "arrayofstructs" ) )
	    fun->org = FUN_ORG_AOS;
    }
    if ( mbuf ) xfree( mbuf );
	...
	switch ( fun->merge ) {
		...
		if ( ( idx =
		       FunColumnLookup( fun, fun->cols[got]->name, 0, NULL,
