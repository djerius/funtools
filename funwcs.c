...
_FunWCS(
	...
	for ( j = 0; j < fun->dims; j++ ) {
		...
		if ( doimage && ( fun->cols[i]->binsiz > 0 ) )
		    ...
		    binsiz = 1.0;
		ival =
		    ( int ) tldim( fun->cols[i]->tlmin, fun->cols[i]->tlmax,
	...
	for ( j = 1; j <= 2; j++ ) {
		...
		if ( ( s = ft_headgets( iheader, "TCRVL", i, NULL, &card ) )
		     && card ) {
		    ft_headappv( oheader, "CRVAL", j, s, NULL );
		    xfree( s );
		}
		if ( ( dval = ft_headgetr( iheader, "TCDLT", i, 0.0, &card ) )
		     && card ) {
		    /* factor in binsize */
		...
		if ( ( s = ft_headgets( iheader, "TCROT", i, NULL, &card ) )
		     && card ) {
		    ft_headappv( oheader, "CROTA", j, s, NULL );
		    xfree( s );
		}
		if ( ( dval = ft_headgetr( iheader, "TCRPX", i, 0.0, &card ) )
		     && card ) {
