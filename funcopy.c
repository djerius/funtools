...
_FunCopy2ImageHeader(
    ...
    ft_headseti( to->header, "BITPIX", 0, from->obitpix, "bits/pixel", 1 );
	...
	case FUN_EVENTS:
	    ...
	    if ( ( s = ft_headgets( to->header, "EXTNAME", 0, NULL, &card ) )
	         && card ) {
		ft_headdel( to->header, "EXTNAME", 0 );
		xfree( s );
	    }
	    if ( ( ft_headgeti( to->header, "EXTVER", 0, 0, &card ) != 0 )
	         && card )
		ft_headdel( to->header, "EXTVER", 0 );
	    if ( ( ft_headgeti( to->header, "EXTLEVEL", 0, 0, &card ) != 0 )
	         && card )
